...
**強ブロック不足ルールにS段位の逃げ道を作るのは、この計器では判定できなかった(2026-09-08、保留)。** 欠陥自体は実在する——`strong_block_shortage`(16枚以上・強ブロック3枚未満)はコア/防御/必要ドロー以外を段位に関係なく見送るが、**ブロックは提示されないカードでは補えないので、このルールには終わり方が無い**。103runのうち82runが不足状態に入り、抜け出せたのは48%だけ。残りは以後の報酬の62%を見送り、S段位109回・A段位95回を捨てている。
