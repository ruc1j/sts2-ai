...
**砂の価値は「砂とHPのどちらが先に尽きるか」で決まる(実測で検証済み)。** 砂余命 = 砂 - 1 ターン、HP余命 = HP ÷ 毎ターン被弾 として、`砂余命 <= HP余命` なら砂が先に尽きる。全runのTHE_INSATIABLE戦18件の最終局面で照合すると、**砂死3件は3件とも「砂が先」、HP死15件は14件が「HPが先」**と正しく分類された(外れた1件はHP32・砂2でHP余命1.4対砂余命1の境界)。砂死3件はいずれもHP33〜35・砂1で、HP余命1.4〜1.7に対し砂余命0だった。
